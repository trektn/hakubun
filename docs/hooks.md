Introduction
============
Hakubun now includes a multi-hook (pseudo-plugin) system for extended functionality.
...
A hook file gets called every time Hakubun's engine emits a signal; this is whether an episode is playing,
...
To use a hook file it's as simple as copying it inside the **~/.hakubun/hooks** directory (create it if doesn't exist).
It will get automatically loaded by Hakubun the next time it starts.
...
There's also an optional special init function that gets called when Hakubun loads the hook:
